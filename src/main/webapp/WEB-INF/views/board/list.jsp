...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif; }
        ...
        .logo { color: #339af0; font-size: 20px; font-weight: 800; display: flex; align-items: center; gap: 8px; } /* 하늘색 포인트 */
        ...
        .filter-list input[type="radio"], .filter-list input[type="checkbox"] { accent-color: #339af0; } /* 하늘색 체크박스 */
        ...
        .location-select:focus { border-color: #339af0; } /* 포커스 색상 */
        ...
        .card { background: transparent; cursor: pointer; transition: transform 0.2s; }
...
<body>
        ...
        <main class="content-area">
            ...
            <div class="gallery-grid">
                <c:forEach items="${list}" var="board">
                    <div class="card move" href='<c:out value="${board.bno}"/>'>
    ...
    <script>
        // 1. 지역 데이터 (시/도에 따른 시/군/구 목록)
        // 주의: 실제로는 행정구역 데이터가 훨씬 많으므로, 필요시 API나 전체 데이터를 사용해야 합니다.
        // 여기서는 예시 데이터만 업데이트합니다.
        const areaData = {
        ...
        $(document).ready(function() {
            ...
            // 페이지 로드 시 기존 선택된 필터 값 복원 (서버에서 넘어온 값)
            ...
            // 2) 지역 복원
            if(savedSido) {
                $("#sido").val(savedSido).trigger("change"); // 변경 이벤트 강제 발생시켜 구군 목록 로드
                if(savedGugun) {
                    setTimeout(function(){
            ...
            // 시/도 select 변경 시 수동으로 change 이벤트 호출 (위의 onchange 속성과 연결)
            $("#sido").on("change", function(){
                categoryChange(this);
            });

            // "필터 적용" 버튼 및 "검색" 버튼 클릭 시 동작
            $("#btnApplyFilter, #searchForm button").on("click", function(e){
                ...
                // 2. 검색 폼 내부의 hidden input에 값 집어넣기
        ...
        function resetLocation() {
            $("#sido").val("");
            categoryChange(document.getElementById("sido")); // 구군 초기화
        }
    </script>

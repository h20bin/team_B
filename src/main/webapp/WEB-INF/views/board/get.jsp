...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Malgun Gothic", sans-serif; }
        ...
        header { width: 100%; height: 64px; display: flex; justify-content: center; align-items: center;
                 position: fixed; top: 0; left: 0; background: rgba(255, 255, 255, 0.95);
                 backdrop-filter: blur(10px); z-index: 1000; border-bottom: 1px solid #f1f3f5; }
        ...
        .main-content { display: flex; gap: 60px; justify-content: center; background-color: #fff;
                        padding: 40px; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); margin-bottom: 40px; }
        ...
        .image-section { width: 100%; height: 500px; border-radius: 16px; overflow: hidden;
        ...
        .thumbnail-item { width: 80px; height: 80px; border-radius: 8px; overflow: hidden; cursor: pointer;
        ...
        #map { width: 100%; height: 350px; border-radius: 12px; border: 1px solid #e9ecef;
               margin-bottom: 40px; background-color: #f8f9fa; }

        .btn-area { margin-top: auto; display: flex; justify-content: flex-end; flex-wrap: wrap; gap: 10px; }
        .btn-list { background-color: var(--main-color); color: white; padding: 12px 24px; border-radius: 8px;
                    font-size: 16px; font-weight: 700; transition: 0.2s; border: none; cursor: pointer;
                    display:inline-block; text-align:center; }
        .btn-list:hover { background-color: #228be6; box-shadow: 0 4px 10px rgba(51, 154, 240, 0.3); }

        /* 툴팁 */
        .copy-tooltip { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
        ...
        /* 리뷰 섹션 */
        .review-section { background-color: #fff; padding: 40px; border-radius: 16px;
                          box-shadow: 0 4px 20px rgba(0,0,0,0.05); }
        .review-title { font-size: 24px; font-weight: 800; margin-bottom: 30px; }

        .review-header { display: flex; align-items: center; gap: 20px; margin-bottom: 20px;
                         padding-bottom: 20px; border-bottom: 1px solid #f1f3f5; flex-wrap: wrap; }
        ...
        .btn-write-review { background-color: var(--main-color); color: white; border: none;
        ...
        .review-form-container { display: none; padding: 25px; background-color: #f8f9fa;
        ...
        .review-form-container textarea { width: 100%; min-height: 80px; padding: 10px;
                                          border: 1px solid #dee2e6; border-radius: 4px; resize: vertical; }
        .review-form-actions { text-align: right; margin-top: 10px; display: flex; gap: 8px; justify-content: flex-end; }
        .review-btn { padding: 10px 20px; border-radius: 8px; font-size: 15px; font-weight: 700;
        ...
        /* 이미지 모달 */
        .modal { display: none; position: fixed; z-index: 2000; padding-top: 100px;
                 left: 0; top: 0; width: 100%; height: 100%; overflow: auto;
                 background-color: rgba(0,0,0,0.9); }
        .modal-content { margin: auto; display: block; width: 80%; max-width: 700px; }
        .close-modal { position: absolute; top: 15px; right: 35px; color: #f1f1f1;
        ...
        <div class="info-section">
            ...
            <div class="location-section">
                <div class="location-text">
                    ...
                    <button type="button" class="btn-copy" onclick="copyAddress()" title="주소 복사">
                ...
                <sec:authorize access="isAuthenticated()">
                    ...
                    <c:if test="${pinfo.username eq board.writer or pinfo.member.auth eq 'ROLE_ADMIN'}">
                        <button type="button" data-oper="modify"
                                class="btn-list"
                                style="background-color:white; color:#339af0; border:1px solid #339af0;">
                            수정
                        </button>
    ...
    <div class="review-section"
        ...
        <div class="review-header">
            ...
            <div class="review-write-btn-area">
                <sec:authorize access="isAuthenticated()">
                    <button class="btn-write-review" id="showReviewFormBtn">
                        <i class="fa-solid fa-pen-to-square"></i> 이용평 작성
                    </button>
                </sec:authorize>
            </div>
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (typeof kakao === 'undefined' || !kakao.maps) {
        ...
        mapContainer.innerHTML =
            '<div style="width:100%; height:100%; display:flex; justify-content:center; align-items:center; color:#fa5252; flex-direction:column; gap:10px;">' +
